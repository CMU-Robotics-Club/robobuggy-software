...
class UKF(Node):
    def __init__(self):
        ...
        self.x_hat = None
        self.Sigma = np.diag([1e-4, 1e-4, 1e-2, 1e-2, 1e-2]) #state covariance
        self.R = self.accuracy_to_mat(50)
        self.Q = np.diag([1e-4, 1e-4, 1e-2, 1e-2, 2.4e-1])
        ...
        self.nand_steering = self.create_publisher(Float64, "other/steeringEstimate", 10)
    ...
    def loop(self):
        if not self.start:
            return
        self.x_hat, self.Sigma = ukf_predict(self.x_hat, self.Sigma, self.Q, None, 0.01, [1.3])
