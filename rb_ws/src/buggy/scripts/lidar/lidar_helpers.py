...
def identify_best_cluster(clusters, boxes):
    ...
    for (cluster, box) in zip(clusters, boxes):
        ...
        angle_weight = 1.0 + 0.5 * np.cos(
            angle)  # front clusters (angle ~ 0) get weight ~1.5, side clusters (angle ~ ±pi/2) get weight ~1.0, back clusters (angle ~ pi) get weight ~0.5
        large_cluster_penalty = 1.0 * max(0, size - 1.0) + 1.0 * max(0,
                                                                     box.get_max_extent() - 3.0)  # penalize clusters larger than 1.0 m^3 or longer than 3.0 m in any dimension
        small_cluster_penalty = 1.5 * max(0, 0.2 - size) + 1.5 * max(0, 0.2 - np.min(
            box.get_extent()))  # penalize clusters smaller than 0.2 m^3 or shorter than 0.2 m in any dimension
